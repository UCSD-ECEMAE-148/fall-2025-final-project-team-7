...
* Created accurate YOLOv8 model for detecting the cars in general, but focusing on the back of cars
  * Model runs on the camera with an approximate 90% accuracy
* We created a ROS2 node on Pi that uses the OAK-D camera running a provided YOLOv8 model to calculate the angle from the center of the camera to the center of the broken down vehicle
...
* We created a second ROS2 node on Pi to control the robot using LiDAR to calculate the distance between the robot and the broken down car
  ...
  * Car starts driving following the road lane when message is received that red blinking light is  detected on “bird’s eye” camera
  * Once the YOLOv8 model detects the broken down car, angle information from camera node is used to navigate the robot with a PD
...
* [Car Dataset](https://universe.roboflow.com/ece-148/car-object-detection-vw2le-5heye/dataset/2)
