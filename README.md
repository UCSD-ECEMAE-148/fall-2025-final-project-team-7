...
* The blink detection algorithm was too sensitive and would trigger on any red light, not just hazard lights on stopped vehicles.
  * To solve this, we combined color thresholding, region-of-interest filtering, and brightness change analysis across frames. We then verified the blinking frequency using a fast Fourier transform (FFT) to confirm the pattern matched typical hazard light blinking. Only when all three conditions were met did the system send the release signal to the mechanic vehicle, reducing false positives significantly.
